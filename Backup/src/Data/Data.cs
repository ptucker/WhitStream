...
namespace WhitStream.Data
{
	...
	public class Punctuation : DataItem
	{
			...
			public Pattern Coerce(Pattern p)
			{
				...
				else if (p is ListPattern)
				{
					...
					for (int i = 0; i < listP.Values.Length && fFound == false; i++)
						fFound = (listP.Values[i].Value == Value);

					if (!fFound)
					{
						List<LiteralPattern> lpNew = new List<LiteralPattern>(listP.Values);
						lpNew.Add(this);
						LiteralPattern[] rglpNew = new LiteralPattern[lpNew.Count];
						lpNew.CopyTo(rglpNew);
        ...
        public class RangePattern : Pattern
		{
            ...
            public Pattern Combine(Pattern p)
            {
                ...
                else if (p is ListPattern)
                {
                    ...
                    List<LiteralPattern> lilp = new List<LiteralPattern>();
		...
		{
			private LiteralPattern[] alo = null;

			/// <summary>
			/// Constructs a list pattern, given a list of LiteralPatterns
			/// </summary>
			/// <param name="rgo">The values to be in the list</param>
			/// <seealso cref="Data.Punctuation.LiteralPattern"/>
			public ListPattern(params LiteralPattern[] rgo)
			{
				alo = rgo;
			}
			...
			public Pattern Coerce(Pattern p)
			{
				...
				else if (p is ListPattern)
				{
					...
					List<LiteralPattern> listP = new List<LiteralPattern>(Values);
					listP.AddRange(listpat.Values);
					LiteralPattern[] rglpNew = new LiteralPattern[listP.Count];
					listP.CopyTo(rglpNew);
					return new ListPattern(rglpNew);
				}
				else if (p is LiteralPattern)
				{
					...
					for (int i = 0; fFound == false && i < Values.Length; i++)
						fFound = Values[i].Equals(litP);

					if (fFound)
						return this;
					else
					{
						List<LiteralPattern> lpNew = new List<LiteralPattern>(Values);
						lpNew.Add(litP);
						LiteralPattern[] rglpNew = new LiteralPattern[lpNew.Count];
						lpNew.CopyTo(rglpNew);
						return new ListPattern(rglpNew);
			...
			public Pattern Uncoerce(Pattern p)
			{
				...
				else if (p is ListPattern)
				{
					ListPattern listpat = p as ListPattern;
					List<LiteralPattern> listP = new List<LiteralPattern>(Values);
					foreach (LiteralPattern lp in listpat.Values)
						listP.Remove(lp);
					LiteralPattern[] rglpNew = new LiteralPattern[listP.Count];
					listP.CopyTo(rglpNew);
					return new ListPattern(rglpNew);
            ...
            public Pattern Combine(Pattern p)
            {
                ...
                else if (p is ListPattern)
                {
                    ...
                    return (lilp.Count > 0) ? (Pattern)new ListPattern(lilp.ToArray()) : (Pattern)new EmptyPattern();
                }
                else if (p is RangePattern)
                {
                    RangePattern rp = p as RangePattern;
        ...
        public static Punctuation Combine(Punctuation p1, Punctuation p2)
        {
            ...
            {
                pRet.AddValue(((Pattern)p1[ip]).Combine((Pattern)p2[ip]));
                if (pRet[ip] is EmptyPattern)
                    return null;
            }
        ...
        public static Punctuation Coerce(Punctuation p1, Punctuation p2)
        {
            ...
            for (int ip = 0; ip < p1.Count; ip++)
            {
                pRet.AddValue(((Pattern)p1[ip]).Coerce((Pattern)p2[ip]));
                if (pRet[ip] is EmptyPattern)
                    return null;
            }
        ...
        public static Punctuation Uncoerce(Punctuation p1, Punctuation p2)
        {
            ...
            for (int ip = 0; ip < p1.Count; ip++)
            {
                pRet.AddValue(((Pattern)p1[ip]).Uncoerce((Pattern)p2[ip]));
                if (pRet[ip] is EmptyPattern)
                    return null;
            }
		...
		/// <returns>The resulting pucntuations from flattening the lists</returns>
				...
				else //ListPattern
				{
					for (int i = 0; i < ret.Count; i++)
					{
						for (int j = 0; j < ((ListPattern)ret[i][iAttr]).Values.Length; j++)
						{
							...
							pRet[iAttr] = ((ListPattern)ret[i][iAttr]).Values[j];
		...
		public byte[] ToByte(Schema schema, int punc_type)
		{
			...
			for (int i = 0; i < alValues.Count; i++)
			{
				...
				else if (pat is ListPattern)
				{
					...
					LiteralPattern[] values = ((ListPattern)pat).Values;
	...
	public class PunctuationScheme
	{
		...
		/// </summary>
		/// <param name="size">The number of attributes if the actual punctuation</param>
