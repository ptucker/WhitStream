...
namespace WhitStream.QueryEngine.Scheduler
{
    ...
    public class PriorityScheduler : Scheduler
    {
        WSThreadPool pool = new WSThreadPool();
        int iNextOp = 0;
        const int NUMBERTOKEEP = 10;
        List<int> nextOp = new List<int>(NUMBERTOKEEP);
        ...
        {
            IResults[] res = base.Init(breakOut, qs);
            pool.Init(ots, NextOperatorPriority);
            return res;
        }
        ...
        private void CalcPriorities()
        {
            nextOp.Clear();
            int priority;
            int[] priorities = new int[NUMBERTOKEEP];
            int[] threads = new int[NUMBERTOKEEP];
            for (int i = 0; i < NUMBERTOKEEP; i++)
                priorities[i] = threads[i] = -1;

            for (int i = 0; i < ots.Count; i++)
            {
                priority = ots[i].QueryOperator.GetItemsReady();
                for (int j = 0; j < NUMBERTOKEEP; j++)
                {
            ...
            nextOp.AddRange(threads);
        }

        private int NextOperatorPriority()
        {
            //int cOp = pool.Length;
            if (iNextOp % NUMBERTOKEEP == 0)
            {
                CalcPriorities();
                iNextOp = 0;
            }
