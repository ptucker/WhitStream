...
namespace WhitStream.QueryEngine.Scheduler
{
    ...
    public class WSThreadPool : IEnumerable<OpThread>
    {
        ...
        private List<OpThread> rgCompleted;
        private List<OpThread> rgDeactivated;
#if DEBUG
        ...
        private List<int> rgLastExecuted = new List<int>(THREADHISTORY);
        ...
        public void Init(List<OpThread> ots, NextOperator n)
        {
            ...
            rgCompleted = new List<OpThread>(ots.Count);
            rgDeactivated = new List<OpThread>(ots.Count);

            rgThread = new Thread[cThreads];

            for (int i = 0; i < THREADHISTORY; i++)
                rgLastExecuted.Add(-1);
        ...
        private void ThreadFunction()
        {
            ...
            //Cache these once, since every access to these functions causes a new memory allocation
            DataItemPool.GetDataItem gdi = dip.GetItem;
            DataItemPool.ReleaseDataItem rdi = dip.ReleaseItem;
            OpThread execThread;
            ...
            do
            {
                ...
                {
                    iEvent = NOp();
                    execThread = this[iEvent];
                    if (execThread.Complete)
                    {
                        rgOpThread.Remove(execThread);
                        rgCompleted.Add(execThread);
                        continue;
                    }
                }
                //UpdateHistory(iEvent);
                execThread.ExecuteOnce(gdi, rdi);

                if (rgOpThread.Count == 0 && rgDeactivated.Count == 0)
                    complete = true;
        ...
        private void UpdateHistory(int iEvent)
        {
            lock (rgLastExecuted)
            {
                rgLastExecuted.RemoveAt(0);
                rgLastExecuted.Add(iEvent);
            }
        }
        ...
        public int GetLastExecuted(int i)
        {
            ...
            {
                if (i < THREADHISTORY)
                    return rgLastExecuted[THREADHISTORY - 1 - i];
                else
                    return -1;
            }
        }
        ...
        {
            get { return rgOpThread[iActive]; }
        }

        /// <summary> How many threads are in the pool? </summary>
        public int Length
        {
            get { return rgOpThread.Count; }
        }
